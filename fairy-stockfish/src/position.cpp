...
bool Position::pseudo_legal(const Move m) const {
  ...
  // YASU: Delayed pawn promotion is a legal same-square move.
  if (promotion_type(m) != NO_PIECE_TYPE &&
      !(type_of(m) == PROMOTION && delayed_pawn_promotion() &&
        from == to && type_of(pc) == PAWN))
      return false;
  ...
  // YASU: Delayed promotion is a same-square pawn move.
  // It changes the pawn into the selected piece without changing squares.
  const bool isDelayedPromotion =
      type_of(m) == PROMOTION &&
      delayed_pawn_promotion() &&
      from == to &&
      type_of(pc) == PAWN;

  // The destination square cannot be occupied by a friendly piece,
  // except for delayed promotion where from == to is intentional.
  if (!isDelayedPromotion && (pieces(us) & to))
      return false;

  // Handle the special case of a pawn move
  if (type_of(pc) == PAWN)
  {
      if (isDelayedPromotion)
      {
          // Delayed promotion is legal only for a pawn already
          // on its promotion rank.
          if (!(promotion_zone(us) & from))
              return false;
      }
      else
      {
          // We have already handled promotion moves, so promotion
          // destination cannot be on the 8th/1st rank.
...
bool Position::gives_check(Move m) const {
  ...
  case PROMOTION:
  {
      ...
      // YASU: Delayed promotion keeps the same square but replaces
      // the pawn with the promoted piece for check detection.
      if (from == to && delayed_pawn_promotion())
          occupied |= to;
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  if (to == from)
  {
      assert((type_of(m) == PROMOTION && (sittuyin_promotion() || delayed_pawn_promotion()))
...
void Position::undo_move(Move m) {
  ...
  assert(type_of(m) == DROP || empty(from) || type_of(m) == CASTLING || is_gating(m)
         || (type_of(m) == PROMOTION && (sittuyin_promotion() || delayed_pawn_promotion()))
         || (is_pass(m) && (pass(us) || var->wallOrMove)));
...
bool Position::is_optional_game_end(Value& result, int ply, int countStarted) const {
  ...
  // YASU CUSTOM CHESS: 50/75-move and repetition draw rules are disabled.
  // The game must continue regardless of rule50/repetition counters.
  (void)result;
  (void)ply;
  (void)countStarted;
  return false;
