...
public:
  ...
  bool delayed_pawn_promotion() const;
...
inline bool Position::delayed_pawn_promotion() const {
  assert(var != nullptr);
  return var->delayedPawnPromotion;
}
...
inline void Position::move_piece(Square from, Square to) {
  ...
  // YASU: delayed promotion uses same-square moves (e8e8q).
  // There is no actual piece movement when from == to.
  if (from == to)
      return;

  Bitboard fromTo = square_bb(from) ^ to;
