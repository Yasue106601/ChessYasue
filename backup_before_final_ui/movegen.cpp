...
namespace {
  ...
  ExtMove* generate_pawn_moves(const Position& pos, ExtMove* moveList, Bitboard target) {
    ...
    const Bitboard standardPromotionZone =
        (pos.sittuyin_promotion() || pos.delayed_pawn_promotion()) ? Bitboard(0) : promotionZone;
    ...
    // Delayed pawn promotions
    // A pawn that has reached the promotion zone may promote
    // on a later turn without changing squares.
    if (pos.delayed_pawn_promotion() && (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS))
    {
        Bitboard promotionPawns = pawns & promotionZone;
        while (promotionPawns)
        {
            ...
            for (PieceSet ps = pos.promotion_piece_types(Us); ps;)
            {
                ...
                *moveList++ = make<PROMOTION>(from, from, pt);
            }
        }
    }
